...
authoring over HTTP. It is based off the {original DAV4Rack framework}[http://github.com/chrisroberts/dav4rack]
adding some useful new features:

* MongoDB(GridFS) support (MongoResource).
* Multi-byte problem fixed.
* Response code problem (207:Multi-Status) fixed (MKCOL,MOVE,COPY).
...
=== Via Gemfile
    ...
    gem 'dav4rack', :git => "git://github.com/exabugs/dav4rack.git"
...
routes.rb add below.
  ...
  mount DAV4Rack::Handler.new(
    :root => '/webdav',
    :root_uri_path => ActionController::Base.config.relative_url_root.to_s + '/grid/content',
    :resource_class => ::DAV4Rack::MongoResource
  ), :at => '/grid/content/'

  mount DAV4Rack::Handler.new(
    :root => Rails.root.to_s + "/tmp",
    :root_uri_path => ActionController::Base.config.relative_url_root.to_s + '/real/content',
